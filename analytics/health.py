...
async def compute_site_health_scores(session: AsyncSession, year: int) -> list[dict]:
    ...
    for row in rows:
        ...
        z_score = (consumption - global_mean) / global_std if global_std > 0 else 0
...
async def compute_enterprise_health_summary(session: AsyncSession, year: int) -> dict:
    scores = await compute_site_health_scores(session, year)
    if not scores:
        return {"overall_health": 0, "sites": []}
